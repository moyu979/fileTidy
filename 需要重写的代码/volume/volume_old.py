...
class Volume:
    ...
    def __init__(self, name=None, id=None, mount_point=None):
        ...
        if mount_point is not None:
            ...
            if not os.path.exists(mount_point):
                ...
                if not os.path.exists(info_path):
                    logging.error(
                        f"记录 {info_path} 的文件夹不存在，给出的挂载点并非一合法的文件管理卷"
                    )
                    raise FileNotFoundError(
                        f"Volume info path {info_path} does not exist."
                    )
                else:
                    ...
                    if temp_id is None:
                        logging.error(
                            f"在挂载点 {mount_point} 中未找到卷id文件，请检查挂载点是否正确"
                        )
                        raise FileNotFoundError(
                            f"No volume id file found in {info_path}. Please check the mount point."
                        )

            # 使用根据挂载点解析的卷id刷新id
            if id is not None and id != temp_id:
                logging.warning(
                    f"提供的卷id {id} 与从挂载点 {mount_point} 解析出的卷id {temp_id} 不一致，可能会导致错误"
                )
                raise ValueError(
                    f"Provided volume id {id} does not match parsed id {temp_id} from mount point {mount_point}."
                )
            else:
                id = temp_id
        ...
        try:
            if id is not None and name is None:
                ...
                logging.warning(
                    "load volume from database by name and id, this is not recommended"
                )
            ...
            if cursor.rowcount == 0:
                logging.error(
                    f"Volume with id {id} and name {name} does not exist in the database."
                )
                raise ValueError(
                    f"Volume with id {id} or name {name} does not exist in the database."
                )
        ...
        try:
            temp = cursor.execute(
                "SELECT * FROM storageStructure WHERE superid=?", (self.values["id"],)
            ).fetchall()
            if len(temp) == 0:
    ...
    def file_detector(self):
        ...
        if self.mount_point == None:
            ...
            conn = sqlite3.connect(conf.get("db_path"))
            cursor = conn.cursor()
            in_db_file = cursor.execute(
                "SELECT * FROM File WHERE volume=?", (self.values["id"],)
            ).fetchall()
            cursor.close()
            conn.close()
            ...
            logging.info(
                f"文件扫描完毕，发现{len(self.matched)}个匹配的文件，{len(self.differed)}个发生变化的文件，{len(self.disappeared)}个消失的文件，{len(self.unexpected)}个不存在于数据库的文件。"
            )
