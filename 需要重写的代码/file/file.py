...
class File:
    ...
    def auto_complete(self):
        ...
        if self.now_path is None:
            self.now_path = self.abspath.replace(
                VolumeFactory.VolumeFactory.get_mount_point(self.volume), ""
            )
    ...
    def append_to_database(self):
        ...
        cursor.execute(
            "INSERT INTO File (md5,size,add_time,from_path,now_path,now_name,volume,state,info) VALUES (?,?,?,?,?,?,?,?,?)",
            (
                self.md5,
                self.size,
                self.add_time,
