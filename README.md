...
* 如果相同，合并，删除重复文件中的一个，并且在删除的文件夹下增加“redirect.txt”文件，记录被删除的文件，此时若文件夹中只有redirect一个文件，将其上移，对所有的重定位项增加前缀，而不是增加第一行目录的前缀。（涉及到多个redirect的合并）

* 如果不相同，则在后手哈希后增加一位到多位的序号，以消除哈希冲突，而后将两个文件分别存入。

经过此步骤，下载的文件夹中的文件的目录哈希值应该完全不同，同时，会生成若干的redirect文件，这些文件不会被记录到哈希表中。

将记录有此次下载所有文件的文件列表与原本存储在afterdownlaod中的列表合并，进行二次去重，方法同一次去重。

此后，未整理的文件中，除redirect外的哈希值唯一，而redirect文件未被计入哈希表。
