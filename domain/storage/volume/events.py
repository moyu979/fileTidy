# CHECK: 待检查 - 领域层 Volume 领域事件 - 卷相关事件定义
...
class VolumeRegistered:
    ...
    def __init__(self, volume: Volume):
        """
        Args:
            volume: 被注册的卷实例，其快照将被保存到事件中。
        """
        snapshot = volume.to_snapshot()
        self.serial = snapshot["serial"]  # 顶层冗余身份字段，便于按 serial 筛日志
        self.volume = snapshot            # 完整快照（聚合）


class VolumeFieldUpdated:
    """卷普通字段更新事件（name / device_id / state / capacity / file_system 等）。

    注意：info 的变更请使用 VolumeInfo* 系列事件，不要用本事件记 field="info"。
    """
...
class VolumeInfoChanged:
    """卷 info 单键变更事件（serial-op-key-old-new 五元组记录）。

    只记录"一个 key 的一次增删改"，本身不包含差异计算逻辑——由上层（应用层）
    判断要发哪个 op、并逐键构造本事件。一次涉及多个 key 的变更建议直接用
    VolumeInfoSet（全量替换）记录，而不是拆成多条本事件。
