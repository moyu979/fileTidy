...
class checker:
    ...
    def check_storage(self,storage):
        ...
        cursor=conn.cursor()
        try:
            datas=cursor.execute("SELECT 1 FORM Storage WHERE id=?",(storage,)).fetchone()
        ...
        if self.fields.get(key,None) is None:
            ...
            if not_allows is not None:
                if value in not_allows:
                    ...
                    if self.fields.get(key,{}).get("must_legal",False):
                        raise ValueError(f"键{key}的值{value}位于不允许出现的值{not_allows}中")
                    else:
                        return None
            ...
            if allows is not None:
                if value not in allows:
                    ...
                    if self.fields.get(key,{}).get("must_legal",False):
                        raise ValueError(f"键{key}的值{value}不位于必须出现的值{allows}中")
                    else:
                        return None
