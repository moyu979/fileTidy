...
## sub_dir
在实际应用中，可能存在一块硬盘为若干个储存池提供存储的情况(举例而言，你偶然拿到了一个16T硬盘，你可能希望它同时为两个8T*4的raid5 阵列提供一部分容量)。在这种情况下，可能需要将一块硬盘分离成两个子阵列，这个时候就存在应用了。

volume直接指向disk 意味着这一步存在一个新建volume的过程