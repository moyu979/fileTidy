...
class Add:
    ...
    def para_convert(self):
        ...
        if self.storagePhysical!=None:
            res=self.cur.execute("SELECT * FROM physicalStorage where volumeName=?",(self.storagePhysical,)).fetchall()
            self.storagePhysical=res[0]
            res=self.cur.execute("SELECT * FROM storageStructure where subid=?",(self.storagePhysical,)).fetchall()
            storageVirtual=res[0]

            if self.storageVirtual!=None:
                res=self.cur.execute("SELECT * FROM virtualStorage where volumeName=?",(self.storageVirtual,)).fetchall()
    ...
    def write_db(self,hash,path,name):
        self.cur.execute("INSERT INTO files (Md5,addTime,fromPath,nowPath,nowName,storageVirtual,storagePhysical) VALUES (?,?,?,?,?,?)",(hash,self.addTime,path,path,name,self.storageVirtual,self.storagePhysical))
