...
class Add:
    ...
    def run(self):
        ...
        for i in files:
            exists[i[3]]=i
            exists_hashkey[i[1]]=i
        Log.writeLog(f"collect {len(exists)} exist files from database")
        ...
        if self.check_exist:
            for i in exists.keys():
                #如果不存在，就计记成删除
                if not os.path.exists(i):
                    f=exists[i]
                    disappear_file.append(i)
                    # self.cur.execute("INSERT INTO change (time,hashMd5,conflictNum,path) VALUES (?,?,?,?)",(self.time,exists[i][1],exists[i][2],"removed"))
                    self.cur.execute("DELETE FROM now WHERE hashMd5=? AND path=?",(exists[i][1],i))
                    check_removed[f[1]]=f
                #如果存在，就查一下哈希
                else:
                    hash=getAHash(i)
                    ...
                    if hash!=exists[i][1]:
                        file=[i,exists[i][1],hash]
                        self.cur.execute("INSERT INTO change (time,hashMd5,conflictNum,path) VALUES (?,?,?,?)",(self.time,exists[i][1],exists[i][2],"removed"))
                        if self.write_origin:
                            self.cur.execute("INSERT INTO origin (time,hashMd5,conflictNum,path) VALUES (?,?,?,?)",(self.time,hash,exists[i][2],i))
                        self.cur.execute("UPDATE now SET hashMd5=? WHERE hashMd5=? AND path=?",(hash,exists[i][1],i))
                        hash_change.append(file[0])
                        Log.writeLog(f"[hash not match]\t{file[0]} \toldhash:{file[1]}\tnewhash\t{file[2]}")
            for f in check_removed.keys():
                del exists[check_removed[f][3]]
                del exists_hashkey[check_removed[f][1]]
        else:
        ...
        for path in not_exist_set:
            ...
            if not hash in exists_hashkey:
                ...
                if os.path.exists(file[3]) and compareFile(file[3],path):
                    if self.write_origin:
                        self.cur.execute("INSERT INTO origin (time,hashMd5,conflictNum,path) VALUES (?,?,?,?)",(self.time,hash,0,path))
                    self.conn.commit()
                    removeFile(path)
                elif not os.path.exists(file[3]):
