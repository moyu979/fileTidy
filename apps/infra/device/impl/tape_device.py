...
class TapeDevice(Device):
    ...
    def check(self):
        print("磁带的检查还没想好怎么做，请手动进行")
        print(f"请输入磁带的检查结果，安全输入0，危险输入1，彻底损坏输入2")
        result=input()
        if result == "0":
            self.orm_model.state = "healthy"
        elif result == "1":
            self.orm_model.state = "danger"
        elif result == "2":
            self.orm_model.state = "fault"
        else:
            print("输入错误，请重新输入")
            return self.check()

        self.orm_model.last_check_time = datetime.datetime.now()
    ...
    def new_device(cls, *args, **kwargs):
        ...
        if path is None and serial is None:
            logger.warning(f"path and serial are both None, generate a new serial")
            serial = generate_id("")
