...
class get_device_data:
    ...
    def get_kind(self):
        ...
        if self.phy_storage.kind in confs.disk_kind.keys():
            self.phy_storage.kind=confs.disk_kind[self.phy_storage.kind]
        if self.phy_storage.kind not in confs.disk_kind.values():
    ...
    def get_id(self):
        if self.kind=="tape":
            ...
            self.phy_storage.id=input("please input disk id:")
            if id=="":
                err=f"try to give an empty id to disk, retry"
                logging.error(err)
                self.get_id()
    ...
    def get_disk_name(self):
        self.phy_storage.disk_name=input("please input disk name, be id if keep empty")
        if self.phy_storage.disk_name=="":
            self.phy_storage.disk_name==self.phy_storage.id
    ...
    def get_health(self):
        ...
        if self.phy_storage.health in confs.health.keys():
            self.phy_storage.health=confs.health[self.phy_storage.health]
        if self.phy_storage.health not in confs.health.values():
