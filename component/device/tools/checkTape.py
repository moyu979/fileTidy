...
def check_tape(device, scan_blocks=False):
    """
    检查HDD健康状态，返回 'health'（健康）、'danger'（警告）、None（检查失败）
    
    Args:
        device: 硬盘设备路径，格式取决于操作系统：
    ...
    if system == "windows":
        from .windows.checkTape import tape_check as impl
    elif system == "linux":
        from .linux.checkTape import tape_check as impl
    elif system == "macos":
        from .macos.checkTape import tape_check as impl
    else:
