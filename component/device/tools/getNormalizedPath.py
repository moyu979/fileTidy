...
def get_normalized_path(dev_path):
    ...
    Returns:
        str: 规范化后的磁盘路径，如果获取失败则返回 None
    """
    system = get_conf_manager().get("system")
    
    if system == "windows":
        from .windows.getNormalizedPath import get_normalized_path as impl
    elif system == "linux":
        from .linux.getNormalizedPath import get_normalized_path as impl
    elif system == "macos":
        from .macos.getNormalizedPath import get_normalized_path as impl
    else:
